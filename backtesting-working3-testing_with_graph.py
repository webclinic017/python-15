...
#    #MACD
    ...
    #PARABOLIC SAR
#    frame['SAR'] = talib.SAR(frame.High, frame.Low, acceleration=0, maximum=0)
...
#            if frame.iloc[i].approach_low: #if price_cross is 1/True which means sell
            if frame.iloc[i].approach_low and (frame.iloc[i].RSI<50) and \
            (frame.MACD.iloc[i-1] > frame.MACDsig.iloc[i-1]): #if price_cross is 1/True which means sell
...
#            if frame.iloc[i].approach_high: #if high approach signal is 1 or true (signal is near the upper band)
            if frame.iloc[i].approach_high and (frame.iloc[i].RSI>50) and \
            (frame.MACD.iloc[i-1] < frame.MACDsig.iloc[i-1]) : #if high approach signal is 1 or true (signal is near the upper band)
    ...
    plt.plot(frame[['Close','rollhigh','rolllow']])
