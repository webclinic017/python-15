...
#    #get 200sma from talib and put it into the frame
    frame['EMA200']=indicators.talib.SMA(frame.Close, timeperiod=in_200ema) #make this variable
    frame['EMA50']=indicators.talib.SMA(frame.Close, timeperiod=in_50ema) #make this variable
    frame['EMA1']=indicators.talib.SMA(frame.Close, timeperiod=in_1ema) #make this variable
    frame['EMA2']=indicators.talib.SMA(frame.Close, timeperiod=in_2ema) #make this variable
...
#            if frame.iloc[i].approach_high: #if high approach signal is 1 or true (signal is near the upper band)
    ...
    print("sma 200",in_200ema)
    print("sma 50",in_50ema)
    print("sma 1",in_1ema)
    print("sma 2",in_2ema)
    ...
    f = open("new_fork_sma.csv", "a", newline="")
...
#190,201,2  50,80,2     20,40,2      2,9,2


for in_200ema in range (221,241,2):
    for in_50ema in range (80,110,2):
        for in_1ema in range (3,4):
            for in_2ema in range (2,3):
                df,symbol,interval,lookback,rollback=getdata('ETHBTC','1h', '2000', 15, 0.996, 1.004, hours_held, in_200ema, in_50ema, in_1ema, in_2ema)
          
# df,symbol,interval,lookback,rollback=getdata('ETHBTC','1h', '20000', 15, 0.996, 1.004, hours_held, 200, 50, 21, 2)
